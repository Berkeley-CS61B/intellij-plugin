...
public class JavadocMethod61bCheck extends AbstractCheck {
    ...
    private static final Pattern MATCH_JAVADOC_ARG =
            CommonUtil.createPattern("@(throws|exception|param)\\s+(\\S+)\\s+\\S*");

    /** compiled regexp to match first part of multilineJavadoc tags * */
    private static final Pattern MATCH_JAVADOC_ARG_MULTILINE_START =
            CommonUtil.createPattern("@(throws|exception|param)\\s+(\\S+)\\s*$");
    ...
    private static final Pattern MATCH_JAVADOC_NOARG =
            CommonUtil.createPattern("@(return|see)\\s+\\S");
    /** compiled regexp to match first part of multilineJavadoc tags * */
    private static final Pattern MATCH_JAVADOC_NOARG_MULTILINE_START =
            CommonUtil.createPattern("@(return|see)\\s*$");
...
//    }
    ...
    private Scope calculateScope(final DetailAST ast) {
        ...
        return ScopeUtil.isInInterfaceOrAnnotationBlock(ast) ? Scope.PUBLIC
                : declaredScope;
    }
        ...
        if (throwsAST != null) {
            ...
            while (child != null) {
                if (child.getType() == TokenTypes.IDENT
                    ...
                    final ExceptionInfo ei = new ExceptionInfo(ast, new ClassInfo(new Token(fi)));
                    retVal.add(ei);
                }
                child = child.getNextSibling();
            }
    ...
    private boolean exceptionMatchedByClass(final List<ExceptionInfo> aThrows,
                                            final Class<?> aClazz) {
        for (ExceptionInfo ei : aThrows) {
            if (aClazz == ei.getClass()) {
                ei.setFound();
                return true;
            } else if (allowThrowsTagsForSubclasses
                    && isSubclass(aClazz, ei.getClass())) {
                return true;
            }
        }
        return false;
     ...
     */
    private boolean isUnchecked(Class<?> exception) {
