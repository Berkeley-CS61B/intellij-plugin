 ...
 * @author lkuehne
 */
public class InnerAssignment61bCheck
        extends AbstractCheck
{
    /**
     * Default list of allowed AST types from an assignement AST node
     * towards the root.
     */
    private static final int[][] BASE_ALLOWED_ASSIGMENT_CONTEXT = {
    ...
    /**
     *  List of additional allowed AST types from an assignment AST node
     *  towards the root.
     */
    private int[][] allowedContext = { };

    /**
     * Adds a set of contexts to the base permissible contexts for assignments.
     * @param strRep a list of lists of tokens, each list separated 
     *        by ":"s 
     */
    public void setAllowedContext(String[] strRep) {
        allowedContext = new int[strRep.length][];
        for (int i = 0; i < allowedContext.length; i += 1) {
            StringTokenizer tokens = new StringTokenizer(strRep[i], ":");
            allowedContext[i] = new int[tokens.countTokens()];
            for (int j = 0; j < allowedContext[i].length; j += 1) {
                allowedContext[i][j] =
                        TokenUtil.getTokenId(tokens.nextToken());
            }
        }
    }
    ...
    {
        if (isInContext(ast, BASE_ALLOWED_ASSIGMENT_CONTEXT)) {
            return;
        }

        if (isInContext(ast, allowedContext)) {
            return;
        }
